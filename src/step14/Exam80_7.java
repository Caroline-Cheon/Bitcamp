 /*주제: 예외처리 - Error 예외의 이점
 => Error 예외를 발생시키는 메서드를 사용할 때
    중간 경우지에 위치한 메서드들은 굳이 예외가 발생한다고 것을 선언할 필요 x
 => 중간 경유지에 잇는 메서드가 Error 예외를 처리하지않으면,
    자동으로 호출자에게 전달된다.
 => 용도!
    스텔스 모드로 최상위 호출자에게 예외를 전달하고 싶을 때 유용하다.
    중간 경유지에 있는 메서드들은 추가로 throws를 선언할 필요가 없다.
    즉 어떤 예외를 던지는지 고민할 필요가 없다. 코딩이 단순해 진다.
