...
public class Exam80_4 {
  ...
  public static void main(String[] args) {
    // 1) Error 예외를 처리하지 않으면 이 메서드의 실행을 즉시 멈추고
    // 상위 호출자에게 예외 객체를 자동 전달한다.
    // ==>main() 메서드의 상위 호출자는? JVM이다.
    //==>JVM은 예외 정보를 받으면 그냥 예외 정보를 꺼내 출력한다.
    // m1();
    ...
    try { 
       m1();
        ...
        System.out.println("오후라...예외 발생");
    } 

    // 3) Error 객체를 던지는 경우, 메서드 선언부에 예외를 던진다고 되어있거나
    // 되어 있지 않거나 동일하게 다룬다. 즉 m1()이나 m2()나 똑같다.
    // => 그럼에도 불구하고 개발자가 m2()를 정의할 때 메서드 선언부에 
    // 어떤 예외를 던지는지 선언했다면,
    // 그것은 다른 개발자에게 자신이 만든 메서드가 어떤 예외를 던지는지 알려주기위함
    //m2();
  }
}
