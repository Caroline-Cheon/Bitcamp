 ...
 *    - excuteQuery() : select 문을 전달
 *    - excuteUpdate() : insert, update, delete 문 전달
 *    - execute() : 모든 sql 문 전달. select 문을 실행할 때 결과를 받기가 번거롭다.
 *                 그래서 가능한 excuteQuery() 또는 executeUpdate()를 사용한다.
 *                 언제 사용하는가? 실행할 sql 문이 select인지 아닌지 구분할 수 없을 때.
 * => java.sql.ResultSet (인터페이스)
 *    - select 문의 실행 결과를 가져오는 객체
 *    - 한 번에 한 개의 레코드(record 또는 row)만 가져온다.
 *    - next(): 서버에서 한 레코드를 가져오는 기능을 한다.
 *    - getXxx(컬럼번호) 또는 getXxx(컬럼명) : 컬럼 값을 꺼낸다.
 *            이 때 컬럼의 타입에 따라 호출하는 메서드를 달리한다.
 */
package step26;
...
public class JDBC02_1 {
  public static void main(String[] args) throws Exception {
    ...
    try (
      ...
      System.out.println(rs.getString("position")); // 가능한 컬럼명을 사용하라!
