...
public class Exam106_1 {
  ...
  static class MyObject extends Thread {
    // 독립적으로 실행할 코드는 run() 메서드를 오버라이딩 한다.
    @Override
    public void run() {
 ...
 * - 보통 응용 프로그램이라고 부른다. 더 줄여서 애플리케이션(응용)이라 부른다.
 * 
 * 
 * "프로세스"
 *=> 실행 중인 프로그램
 ...
 *    CPU의 작업처리가 워낙 빠르기 때문에, 겉으로는 동시에 실행하는 것처럼 보인다.
 ...
 2) 멀티 스레드 방식
  => 원본 프로세스의 힙(heap) 메모리(자바는 Method Area 영역 포함)는 공유하고,
     스택 메모리만 스레드 별로 관리한다.
  => 따라서 멀티 프로세스 방식보다 메모리가 절약된다.
  => 스레드의 특성상 프로세스에 종속되기 때문에
     프로세스를 종료하면 그 프로세스에 종속된 모든 스레드도 같이 제거되기 때문에
     관리하기 쉽다.
 ...
 *    CPU 사용을 뺏길 때까지 run() 메서드 안에 기록된 코드를 실행한다.
