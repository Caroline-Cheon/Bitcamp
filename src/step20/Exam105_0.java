...
=> 한 번 연결하면 연결을 종료할 때까지 계속 데이터를 주고 받는 방식
=> 연결하기 --> 요청 --> 응답 --> 요청 --> 응답 --> 요청-->.....-->연결끊기
=> 연결된 동안 수행한 작업 정보를 계속 서버에 유지한다.
   그래서 상태 정보를 유지한다는 의미로 "stateful"방식이라고 한다.
=> 장점
   - 클라이언트의 작업 정보를 유지하기 때문에 영속성 있는 작업 처리가 가능하다.
=> 단점
   - 클라이언트가 연결되면 그 클라이언트 접속 정보를 계속 서버 메모리에 유지해야 하기 때문에
     메모리를 많이 차지한다.
   - 많은 클라이언트의 요청을 처리할 수 없다.
   - 클라이언트가 사용하지 않더라도 연결을 끊지 않으면 서버는 계속 그 클라이언트의 정보를 계속 유지해야 한다.
   - 메모리 낭비가 심하다.
=> 예)
   FTP, 채팅, 게임 등, 화상통신, Telnet
   자바 예제로 만든 EchoClient/EchoServer, GameClient/GameServer
      
 2) connecionless(=stateless)
=> 서버에 작업을 요청할 때 마다 연결하고 응답을 받으면 즉시 끊는다.
=> 연결하기 --> 요청 --> 응답 --> 연결끊기, 연결하기 --> 요청 --> 응답 --> 응답끊기 
=> 작업을 수행한 후 연결을 바로 끊기 때문에 작업 상태를 서버에 저장하지 않는다.
   그래서 "stateless"방식이라 부른다.
=> 장점
   - 응답을 한 후 바로 연결을 끊기 때문에, 서버는 보다 많은 클라이언트의 요청을 처리할 수 있다.
   - 적은 메모리로 많은 요청을 처리할 수 있다.
=> 단점
   - 클라이언트의 정보를 유지하지 않기 때문에 중간 작업 결과를 서버에 보관하지 않는다.
     따라서 영속성 있는 작업을 할 수 없다.
=> 예)
   114 안내, 메신저, 웹,(http)
