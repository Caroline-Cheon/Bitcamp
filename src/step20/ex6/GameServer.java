...
public class GameServer {
  ...
  public static void main(String[] args) throws Exception {
    ...
    while  (true) {
      ...
      t.start();  // 스레드를 독립적으로 실행시킨다. 주의! run()을 직접 호출하는 것이 아니다.
...
//    ss.close();
  ...
  static class MyThread extends Thread {
    ...
    @Override
    public void run() {
      try (Scanner socketIn = new Scanner(new BufferedInputStream(socket.getInputStream()));
          PrintStream socketOut = new PrintStream(new BufferedOutputStream(socket.getOutputStream()), true, "UTF-8");) {
