...
public class Exam64_0 {
  static {
    System.out.println("Exam64_0: 스태틱 초기화 블록 실행");
  }
  static class MyType {
    static int a;
    int b;
    static {System.out.println("MyType: 스태틱 초기화 블록 실행");
    }
    static void m1() {
  ...
  public static void main(String[] args) {
    ...
    //MyType.a = 100;  //MyType 클래스 로딩 --> 스태틱 변수 생성 --> 스테틱 초기화 블록 실행
    ...
    //new MyType();
    //new MyType();   // 한번 클래스가 로딩되면 중복해서 로딩되지 않는다.
...
# main()이 스테틱인 이유?
- JVM이 클래스를 실행할 때 해당 클래스를 로딩하고 바로 main()을 호출할 수 있도록 하기 위함
