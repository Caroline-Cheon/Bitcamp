 ...
 * 2) 로그 출력 방법을 설정
 *    - log4j.appender.로그출력명=출력 방법을 클래스 이름으로 지정한다.
 *    - 출력 방법
 ...
 *      지정된 파일로 출력하기
 *        
 *    > 네트웍 출력 => org.apache.log4j.SocketAppender
 *     원격 컴퓨터로 출력
 ...
 *      > 특정 패턴으로 출력 => org.apache.log4j.jdbc.PatternLayout
 *        예) DEBUG [main]: 메시지 1
 ...
 *          메시지..
 *        </log4j:eventset>
 * 4) PatternLayout의 출력 형식을 지정하는 방법
 *    - %p  : 로그 레벨을 출력한다. 예)DEBUG, FATAL, INFO
 *    - %n  : 줄바꿈 코드 넣기
 *    - %m  : 로그 메시지를 출력한다.
 *    - %숫자p : 중간 숫자는 자릿수를 의미한다. 오른쪽 정렬
 *    - %-숫자p : 중간 숫자는 자릿수를 의미한다. 왼쪽 정렬
 *    - %t : 스레드 이름을 출력한다.
 *    - %c : 로그분류명을 출력한다.
 *           로그분류명?
