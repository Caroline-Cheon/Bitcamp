 ...
 * - 출력 레벨
 *   FATAL  : 애플리케이션을 중지시켜야 하는 심각한 오류일 때, 이 레벨로 로그를 남겨라!
 ...
 *   DEBUG  : 애플리케이션 내부 실행 상태를 남기고 싶을 때, 이 레벨로 로그를 남겨라!
 *   TRACE  : 아주 상세한 내부 실행 상태를 남기고 싶을 때, 이 레벨로 로그를 남겨라!
 * - 출력 레벨을 설정하면 다음 규칙에 따라 하위 레벨의 로그도 함께 출력한다.
 *   TRACE > DEBUG > INFO > WARN > ERROR > FATAL
 *   예) 로그 레벨을 DEBUG로 설정하면 그 하위 레벨인 INFO, WARN, ERROR, FATAL 로그까지 출력한다.
 * 
 */
