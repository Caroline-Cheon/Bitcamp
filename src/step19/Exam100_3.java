...
=> 결론
   ...
   바이트 값을 그래도 출력하고 그대로 읽는다.

   캐릭터 스트림(Reader/Writer 계열 클래스)은
   출력하는 데이터는 플랫폼 기본 문자 코드로 자동 변환되고,
   읽는 데이터는 자바 플랫폼 기본 문자코드(UTF-16)으로 자동 변환된다.
   ...
   클래스를 사용하라!
   왜? 중간에서 자동변환 과정이 없이, 순수하게 데이터를 읽고 쓴다.

=> 텍스트 파일과 바이너리 파일을 구분하는 방법?
   - 메모장으로 읽어 들인 후 변경하여 올바른 형식으로 저장할 수 잇다면
     텍스트 파일이다.
...
// 41 42 43 61 62 63 30 31 32 B0 A1 B0 A2 B0 A3    //hexdump
// 41 42 43 61 62 63 30 31 32 ac00 ac01 ac04        //출력